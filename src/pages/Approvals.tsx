...
export default function Approvals() {
  ...
  if (!appUser || (appUser.role !== 'superadmin' && appUser.role !== 'partner')) {
    return (
    ...
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      <div className="bg-white shadow rounded-lg overflow-hidden border border-gray-200">
        <div className="px-4 py-5 border-b border-gray-200 sm:px-6">
          ...
          <p className="mt-1 text-sm text-gray-500">Permintaan edit data tamu dari Client yang menunggu persetujuan Anda.</p>
        </div>
              ...
              <li key={req.id} className="p-4 sm:p-6 hover:bg-gray-50">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4 text-sm border border-gray-100 rounded-lg p-4 bg-white">
                      <div>
                      ...
                      <div>
                        ...
                        <div className="space-y-1">
                           <p className={req.originalData.name !== req.requestedData.name ? 'font-medium text-indigo-700' : ''}><span className="text-gray-400">Nama:</span> {req.requestedData.name || '-'}</p>
                           <p className={req.originalData.phone !== req.requestedData.phone ? 'font-medium text-indigo-700' : ''}><span className="text-gray-400">HP:</span> {req.requestedData.phone || '-'}</p>
                           <p className={req.originalData.address !== req.requestedData.address ? 'font-medium text-indigo-700' : ''}><span className="text-gray-400">Alamat:</span> {req.requestedData.address || '-'}</p>
                           <p className={req.originalData.category !== req.requestedData.category ? 'font-medium text-indigo-700' : ''}><span className="text-gray-400">Kategori:</span> {req.requestedData.category || '-'}</p>
                           <p className={req.originalData.session !== req.requestedData.session ? 'font-medium text-indigo-700' : ''}><span className="text-gray-400">Sesi:</span> {req.requestedData.session || '-'}</p>
                        </div>
                      </div>
                    </div>
                  </div>
